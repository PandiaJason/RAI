...
def main():
    ...
    os.makedirs("./data/v0.6_rl_checkpoints/", exist_ok=True)
    torch.save(model.state_dict(), "./data/v0.6_rl_checkpoints/rai_v6_alpha.pt")
    ...
    for label, df in [("2020-2024 Out-of-Sample", test_df), ("2010-2019 Historical", train_df)]:
        ...
        print(f"  🏆 RAI v6 ALPHA Final:   ${m['final']:,.2f}  (Net Profit: ${m['final']-10000:+,.2f})", flush=True)
        print(f"  🏆 RAI v6 ALPHA Return:  {m['return_pct']:+.2f}%", flush=True)
        print(f"  🏆 RAI v6 ALPHA Sharpe:  {m['sharpe']:.2f}", flush=True)
        print(f"  🏆 RAI v6 ALPHA Max DD:  {m['max_dd_pct']:.2f}%", flush=True)
        print(f"  ---------------------------------------------------------", flush=True)
